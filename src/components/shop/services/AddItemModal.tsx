...
import { createShopItem } from "@/lib/actions/shopServices";
        ...
        <div className="flex justify-between items-center p-4 border-b bg-white shrink-0">
          ...
          <div className="flex gap-3">
            <div className="px-3 py-1 bg-slate-100 text-xs font-medium text-slate-500 rounded-md border flex items-center">
              {submitting ? "Saving..." : "Unsaved product"}
            </div>
            <button
              type="submit"
              disabled={submitting}
              className="px-6 py-2 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 disabled:opacity-50 transition-all font-sans tracking-wide"
            >
              SAVE PRODUCT {submitting && "..."}
            </button>
          </div>
        </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                      <p className="text-xs text-slate-500 leading-relaxed font-medium">
                        This item will be created as a single product. You can add variations (like size or color) at any time by clicking the button above.
                      </p>
                    </div>
                  </div>
                ) : (
...
function combinationsExist(options: Option[]) {
  return options.some(o => o.values.some(v => v.selected));
}
