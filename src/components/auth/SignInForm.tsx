...
import { loginServer } from "@/lib/actions/auth";

const initialState = { error: "", redirectTo: undefined };

export default function SignInForm() {
  const router = useRouter();
  const [state, formAction, isPending] = useActionState(loginServer, initialState);
          ...
          <div>
            ...
            <form action={formAction}>
              <div className="space-y-6">
                ...
                {state?.error && (
                  <p className="text-sm text-center text-error-500">{state.error}</p>
                )}
