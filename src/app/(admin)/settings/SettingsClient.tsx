...
export default function SettingsClient({ initialShop, error: initialError }: { initialShop: any; error?: string }) {
    const { selectedShopId, selectedShop, isLoading: isShopLoading, refreshShop } = useShop();

    const [isSaving, setIsSaving] = useState(false);
    const [saveMessage, setSaveMessage] = useState<string | null>(null);

    // Use initialShop for first render to avoid loading spinner
    const shopData = selectedShop || initialShop || {};
    ...
    // Sync local state if selectedShop changes after initial load (e.g. user switches shops)
    useEffect(() => {
        if (selectedShop && selectedShop.id !== initialShop?.id) {
    ...
    const activeShopId = selectedShopId || initialShop?.id;
